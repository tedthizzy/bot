#!/usr/bin/env bash
# deploy/preflight.sh — everything that must be true before `systemctl start
# rover.target`. ARCHITECTURE 11 step 11.
#
#   sudo /opt/rover/deploy/preflight.sh
#
...
# rover-cam holds the camera and robotd holds /dev/rover-mcu. With either up,
...
if [ -e "$DEV" ]; then
  ...
  echo "    check dtoverlay=uart5 in /boot/firmware/config.txt and the udev rule:"
  ls -l /sys/class/tty/ttyAMA*/device 2>/dev/null | sed 's/^/    /' || true
fi

# --- the MCU ----------------------------------------------------------------
# The banner is a B protocol frame on UART1, not console text: a release build
# prints nothing on USB at all. Everything below reads one banner and one
# telemetry frame and asserts against [safety].

head_ "MCU banner and safety cross-check"
...
from rover_contracts.serial_codec import (
    SAFETY_HASH_KEYS, BootFrame, CtrlFlag, DecodeOk, EventCode, EventFrame,
    FrameReader, TelemetryFrame,
)

dev, cfg_path = sys.argv[1], sys.argv[2]
cfg = load_config(cfg_path)
reader, seen, deadline = FrameReader(), {}, time.monotonic() + 8.0
# T streams at 50 Hz from boot whether or not an H has arrived, and B repeats
# at 1 Hz until the first valid H, so listening is enough: preflight never
# writes to this port.
with serial.Serial(dev, cfg.serial.baud, timeout=0.2) as port:
    while time.monotonic() < deadline and not {"B", "T"} <= seen.keys():
        for r in reader.feed(port.read(4096)):
            if isinstance(r, DecodeOk):
                seen.setdefault(type(r.frame).TYPE, r.frame)
                if isinstance(r.frame, EventFrame) and r.frame.event == EventCode.CAL_STORED:
                    seen["CAL"] = r.frame

bad = 0
banner = seen.get("B")
if banner is None:
    print("    no B banner in 8 s — is the MCU flashed with a release build?")
    raise SystemExit(1)
assert isinstance(banner, BootFrame)
print(f"    fw_ver={banner.fw_ver} proto={banner.proto_ver} caps=0x{banner.caps:04X} "
      f"reset_reason={banner.reset_reason} session={banner.session}")

want = cfg.safety.safety_hash()
if banner.safety_hash != want:
    bad = 1
    print(f"    safety_hash MISMATCH: MCU {banner.safety_hash}, [safety] {want}")
    print("    [safety] mirror keys, in the hashed order:")
    for k in SAFETY_HASH_KEYS:
        print(f"      {k} = {getattr(cfg.safety, k)}")
    print("    The MCU's set is compiled in; edit firmware/core, not the TOML.")
else:
    print(f"    safety_hash {want} matches the seven [safety] mirror keys")

tel = seen.get("T")
if tel is None:
    bad = 1
    print("    no T telemetry — it should stream at 50 Hz from boot")
else:
    assert isinstance(tel, TelemetryFrame)
    flags = CtrlFlag(tel.ctrl_flags)
    print(f"    ctrl_flags=0x{tel.ctrl_flags:03X} {flags!s}")
    if CtrlFlag.DEBUG_BUILD in flags:
        bad = 1
        print("    b7 debug_build SET — a release build must not (I-18, A37)")
    if CtrlFlag.CAL_VALID not in flags:
        bad = 1
        print("    b6 cal_valid CLEAR — the MCU self-baselines on entry to DISARMED")
    for bit, name in ((CtrlFlag.TOF_FL_OK, "front-L"), (CtrlFlag.TOF_FR_OK, "front-R")):
        if bit not in flags:
            bad = 1
            print(f"    ToF {name} not ok — one dead sensor halves the cone (I-16)")

# obstacle_escalate_s and r_pack_mohm mirror firmware constants but are not in
# the seven the hash covers, so they can drift silently. Printed, and the
# escalation window is asserted against the E FAULT_SET that follows a wall
# approach only at G4 -- here they are at least visible beside the hash.
print(f"    unhashed [safety] mirrors: obstacle_escalate_s="
      f"{cfg.safety.obstacle_escalate_s} r_pack_mohm={cfg.safety.r_pack_mohm} "
      "(no safety_hash coverage; firmware/core/rover_config.h is the source)")

cal = seen.get("CAL")
if cal is None:
    print(f"    no E CAL_STORED seen; assert it against "
          f"cliff_baseline_mm={cfg.safety.cliff_baseline_mm} after a DISARM")
elif cal.arg != cfg.safety.cliff_baseline_mm:
    bad = 1
    print(f"    cliff baseline MISMATCH: MCU {cal.arg} mm, "
          f"[safety] cliff_baseline_mm {cfg.safety.cliff_baseline_mm}")
else:
    print(f"    cliff baseline {cal.arg} mm matches [safety] (asserted, never written)")

raise SystemExit(bad)
PY
  then ok "banner parsed; safety_hash, cal_valid, ToF coverage and ctrl_flags b7 asserted"
  else bad "MCU cross-check failed — detail above, with both constant sets on a hash mismatch"
  fi
  # The achieved ToF timing budget and inter-measurement period are read back
  # per sensor over I2C, which only the firmware can do; G2 asserts them
  # against [safety] and open item 17 carries the long-mode fallback.
  skip "per-sensor ToF timing budget/period read-back — a G2 bench check, not a Pi-side one"
else
  skip "no $DEV, so no banner"
fi
