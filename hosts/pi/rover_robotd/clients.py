"""Bus clients: connection-bound identity, the replay window, and the current
instruction.
...
disconnect or an e-stop and never rolls back.  Nothing in this module knows
about the rover link; the wire has no session of its own any more (ADR-0013).
"""
...
log = logging.getLogger("rover.robotd.clients")
...
def resolve_uid(user: str) -> int | None:
    ...
    The per-unit users exist on the Pi and not on a development machine, so an
    unresolvable name disables the uid row rather than refusing every
    connection.
    """
    import pwd
