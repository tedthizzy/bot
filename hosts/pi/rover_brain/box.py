...
from rover_contracts.messages import SkillCall, SkillName
...
from rover_contracts.skills import CATALOG
...
_PROSE: Final = frozenset({"title", "description"})
"""What pydantic adds to a JSON schema that a grammar has no use for."""
...
def _inline(spec: Mapping[str, Any], defs: Mapping[str, Any]) -> dict[str, Any]:
    """One property as the grammar wants it: ``$ref`` resolved (pydantic emits
    the ``Face`` enum as a definition) and the prose dropped."""
    if "$ref" in spec:
        spec = defs[str(spec["$ref"]).rsplit("/", 1)[-1]]
    return {key: value for key, value in spec.items() if key not in _PROSE}


def _branch(skill: str, model: type[BaseModel]) -> dict[str, Any]:
    """One ``oneOf`` branch, ``speech`` first so it decodes first and feeds
    sentence-streamed TTS.  ``args`` is the argument model's own JSON schema,
    so a bound the grammar constrains is the bound stage two enforces."""
...
SKILL_CALL_SCHEMA: Final[dict[str, Any]] = {
    "oneOf": [
        _branch(spec.name, spec.model_args)
        for spec in CATALOG
        if spec.model_args is not None
    ]
}
"""The strict profile: a top-level ``oneOf`` over the catalog's seven model
skills, every object ``additionalProperties: false``, every bounded numeric an
integer (A11).  Generated from the contracts' argument models, so what ``make
schemas`` writes to ``box/schema/skillcall.json`` is what stage two validates,
