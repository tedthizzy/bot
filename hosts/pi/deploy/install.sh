#!/usr/bin/env bash
# hosts/pi/deploy/install.sh — bring a clean Raspberry Pi OS Trixie 64-bit Lite card to
...
say "apt packages"
packages=$(sed -e 's/#.*//' -e '/^[[:space:]]*$/d' "$ROOT/hosts/pi/deploy/apt-packages.txt" | tr -s '[:space:]' ' ')
info "$packages"
...
install -m 0644 "$ROOT/hosts/pi/deploy/udev/99-rover.rules" /etc/udev/rules.d/99-rover.rules
udevadm control --reload-rules
udevadm trigger --subsystem-match=tty

for u in "${UNITS[@]}"; do
  install -m 0644 "$ROOT/hosts/pi/deploy/systemd/$u.service" "/etc/systemd/system/$u.service"
done
install -m 0644 "$ROOT/hosts/pi/deploy/systemd/rover.target" /etc/systemd/system/rover.target
...
# happen: rover-cam would hold the camera and robotd would hold /dev/serial0,
...
# The WAVE ROVER's driver board is wired to the Pi header's UART0 (GPIO 14/15),
# which Bluetooth owns by default on a Pi 4. disable-bt hands the PL011 back to
# the header so /dev/serial0 is a real UART, not the mini one whose baud drifts
# with the core clock. This is the same change Waveshare's own setup makes.
# The serial console must also leave that port, or the kernel talks over the
# rover's command stream.
...
# The serial console shares UART0 with the rover. Remove it from cmdline.txt
# and stop the service that would attach Bluetooth to the same port.
CMDLINE_TXT=${CMDLINE_TXT:-/boot/firmware/cmdline.txt}
if [ -f "$CMDLINE_TXT" ] && grep -qE 'console=(serial0|ttyAMA0),[0-9]+' "$CMDLINE_TXT"; then
  sed -i -E 's/console=(serial0|ttyAMA0),[0-9]+ ?//' "$CMDLINE_TXT"
  info "removed the serial console from $CMDLINE_TXT"
  changed=1
else
  info "no serial console on UART0"
fi
...
cat <<EOF
  Next, in order:
    1. plug the driver board's UART cable into the Pi header, power the rover,
       and read the firmware banner:
         $VENV/bin/python -m rover_devtools.wirecat /dev/serial0
    2. sudo $ROOT/hosts/pi/deploy/preflight.sh
