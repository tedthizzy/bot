...
What stayed here: the `.gitignore` rules that ignore everything a gate writes
into `logs/gates/`, which `GateRun` creates on first use, so a gate result is
...
in a wav file later touches only `FillerPlayer`. Because there are no assets, the
tiers and the completion table are one module, `rover_brain/filler.py`, not the
`filler/` directory §12 draws, and the `filler/*.wav` package-data glob is gone.
