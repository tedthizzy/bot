...
It never touches the serial port, it is in no stop path except as an additional
stop authority, and every send is best effort: with robotd or brain absent the
pages still load and the rover behaves identically.  Lines from robotd and brain
are relayed to the browser verbatim; rover-web does not interpret them.
"""
...
LATCHED_FAULT_BITS: dict[str, int] = {
    name: int(Fault[name.upper()])
    for name in (bit.name.lower() for bit in Fault if bit & LATCHED_FAULTS)
}
"""Bus name -> bit for the latched fault class, for the face page's recovery
panel.  Every one of them is a ``ClearableFault`` and reaches the MCU's ``C``."""
_LINE_LIMIT = 1 << 17
_RELAY_DEPTH = 256
_RECONNECT_S = 1.0
_TELEOP_HZ = 20.0
...
class TeleopStream:
    ...
    scales it by the configured twist limits, so a browser cannot name a value
    out of bounds.  Once the last stick message ages past
    ``teleop_input_max_age_ms`` the stream sends **one** zero twist and stops
    entirely: a backgrounded tab must not renew the last non-zero value.
    """
    ...
    def _send(self, x: float, y: float) -> None:
        self._client.send(
            TwistMessage(
                ...
                twist=TwistPayload(
                    linear_x_mps=round(y * self._linear, 4),
                    angular_z_radps=round(x * self._angular, 4),
                ),
            )
        )
...
async def _post_clear(request: web.Request) -> web.Response:
    """Recovery from any latched fault; ``[bus] clear_sources`` defaults to ``web``.

    The body is optional and names the faults to clear, defaulting to the
    software e-stop.  Without the body form, a `WDT_REBOOT` or `BROWNOUT` latch
    -- I-20's own designed outcome -- left the rover dead to voice, web and
    teleop with no user-facing control that reached it.  Nothing new is
    trusted: robotd masks the list with ``LATCHED_FAULTS`` and the MCU refuses
    a bit whose cause is still present.
    """
...
async def _status(request: web.Request) -> web.Response:
    ...
    ``latched_faults`` is the bit table the recovery panel decodes
    ``state.mcu.fault`` with, served from ``rover_contracts`` rather than
    restated in the page: the class definition has one home (5.1).
    """
    ...
    return web.json_response(
        {
            ...
            "latched_faults": LATCHED_FAULT_BITS,
        }
    )
...
async def _websocket(request: web.Request) -> web.WebSocketResponse:
    ...
    stream = (
        ...
        if teleop is None
            ...
            teleop,
            linear_mps=app[CONFIG].limits.twist_linear_mps,
            angular_radps=app[CONFIG].limits.twist_angular_radps,
