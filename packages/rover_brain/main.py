...
from rover_contracts.units import m_to_cm, mps_to_cms, rad_to_deg
from rover_contracts.worldstate import MotionBudget, PoseCm, WorldState
...
from rover_brain.validate import MOTION_CAPABLE, crosses_bus, to_bus_message
...
class RobotdClient:
    ...
    def moving(self) -> bool:
        return self.state is not None and self.state.mcu.motion

    @property
    def heading_deg(self) -> float:
        return 0.0 if self.state is None else rad_to_deg(self.state.pose.yaw_rad)
...
class BrainApp:
    ...
    async def _plan(self, text: str, turn_id: str) -> None:
        ...
        stop, forward, back, left and right (4.6)."""
        local = route(text, self._router_defaults)
    ...
    async def _execute(self, call: SkillCall, turn_id: str) -> None:
        ...
        self.post(F.Executed(turn_id=turn_id, status=status, reason=reason))
    ...
    def _budget_left(self, state: StateMessage | None) -> MotionBudget:
        """I-15's remaining budget, read from robotd's published ledger.

        robotd charges measured odometry and measured motion seconds; a second
        accounting here -- charging a goal deadline that carries 1.5x slack plus
        0.5 s, for non-motion skills too -- reported a budget the enforcer never
        applied.  Before the first state message, the configured limits are the
        honest answer.
        """
        limits = self.config.limits
        path_m = limits.budget_path_m if state is None else state.budget.path_m
        motion_s = limits.budget_motion_s if state is None else state.budget.motion_s
        return MotionBudget(path_cm=int(path_m * _CENTIMETRE), seconds=int(motion_s))

    def _world_state(self) -> WorldState:
        state = self.robotd.state
        front_cm, at_max, obstacle = 0, False, True
        pose, heading, battery, bumper, moving = PoseCm(x=0, y=0), 0, 0, False, False
        if state is not None:
            pose = PoseCm(x=m_to_cm(state.pose.x_m), y=m_to_cm(state.pose.y_m))
            heading = int(round(rad_to_deg(state.pose.yaw_rad)))
            battery = state.battery.pct
            bumper = state.bumper
            moving = state.mcu.motion
            at_max = state.front_at_max
            front = state.ranges_m.front
            # A null front range is the 65535 sensor-error sentinel: stale
            # sensors are blockage, not clear path (I-16).
            measured = 0 if front is None else int(front * _CENTIMETRE)
            front_cm = 600 if at_max else measured
            obstacle = front is None or front_cm <= self.config.safety.tof_slow_mm / 10
        unlocked = at_max or front_cm > 100
        limits = self.config.limits
        cap_mps = limits.speed_mps if unlocked else limits.speed_default_mps
        return build_world_state(
            pose_cm=pose,
            heading_deg=int(((heading + 180) % 360) - 180),
            battery_pct=battery,
            obstacle_ahead=obstacle,
            front_range_cm=min(600, max(0, front_cm)),
            front_at_max=at_max,
            bumper=bumper,
            moving=moving,
            speed_cap_cms=mps_to_cms(cap_mps),
            budget=self._budget_left(state),
            allow_motion=self.fsm.authorized_motion,
            last_result=self._last_result,
            last_scene=self.scene.last_scene,
            recently_seen=self.scene.recently_seen(self.robotd.heading_deg),
        )


class _BusTurns:
    """``find``'s motion primitive: a ``turn`` skill on the current turn id,
    ...
    async def turn(
        ...
        call = TurnCall(
            ...
            skill="turn",
            args=TurnArgs(
                angle_deg=int(round(max(-180.0, min(180.0, angle_deg)))),
                rate_dps=int(round(max(5.0, min(60.0, rate_dps)))),
            ),
        )
