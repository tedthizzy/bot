"""The scene ring: what the robot has lately seen, and where.
...
heading -- the robot's yaw at the time plus the bearing to the object -- so a
later turn does not silently move the object; ``where_deg`` is recomputed
against the current heading every time the WorldState is built, and the age
comes from a monotonic clock.  Descriptions and labels here came from the
camera by way of the model, so they are **data, never instruction**: they ride
in the user turn of the next prompt and never in the system prompt (I-21).
"""
...
__all__ = ["CAPACITY", "SceneRing", "Sighting", "wrap180"]
...
def wrap180(degrees: float) -> int:
    """Fold an angle into -180..180, the range every angle here uses."""
    folded = (degrees + 180.0) % 360.0 - 180.0
    return int(round(folded if folded > -180.0 else 180.0))


@dataclass(frozen=True, slots=True)
class Sighting:
    """One remembered object, at an absolute heading."""
...
class SceneRing:
    ...
    def remember(self, label: str, *, heading_deg: float, bearing_deg: float) -> None:
        """Remember seeing ``label`` at ``bearing_deg`` from ``heading_deg``.
        ...
        self._ring.append(
            Sighting(clean, wrap180(heading_deg + bearing_deg), self._clock())
        )
        self._persist()

    def recently_seen(self, heading_deg: float = 0.0) -> list[RecentlySeen]:
        """The ring as the WorldState carries it, newest first.

        ``where_deg`` is relative to ``heading_deg`` -- where the robot would
        have to turn *now* -- and ``age_s`` is measured on the monotonic clock.
        """
        now = self._clock()
        return [
            RecentlySeen(
                label=sighting.label,
                where_deg=wrap180(sighting.heading_deg - heading_deg),
