...
A29/ARCHITECTURE 4.3: brain must never play TTS while the MCU is
``ARMED_MOVING``.  That is enforced once, in :class:`HalfDuplexTts`, rather
than repeated in each backend.
"""
...
"""How often HalfDuplexTts re-reads mcu.state while a sentence is playing."""
...
class HalfDuplexTts:
    """A29: no TTS plays while ``mcu.state == ARMED_MOVING``.
