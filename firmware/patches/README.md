...
describes. Every added line carries a `bot:` comment or lives in one of the two
new files, `General_Driver/bot_config.h` and `General_Driver/bot_safety.h`.
...
`T:900` (`mainType` / `moduleType`), missions and file commands, servo and arm
commands, `T:138` speed rate, `T:605` debug print level, `T:600` reboot and
`T:604` NVS erase are left as upstream has them. The host never sends them
(`docs/protocol.md`), and none of them can raise the cap, lengthen the
heartbeat or bring a radio up: the cap sits in the PWM write path, the
heartbeat guard is in `changeHeartBeatDelay()`, and the radio code is not
compiled in.
