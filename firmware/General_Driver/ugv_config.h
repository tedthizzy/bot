// bot: fork constants (heartbeat, power cap, radios, sensors), one file to audit.
...
byte espNowMode = BOT_ESPNOW_ENABLED ? 3 : 0;  // bot: none when ESP-NOW is compiled out
...
bool ctrlByBroadcast = BOT_ESPNOW_ENABLED ? true : false;  // bot: never when compiled out
...
int HEART_BEAT_DELAY = BOT_HEARTBEAT_MS;  // bot: 300 ms; changeHeartBeatDelay() can only lower it
